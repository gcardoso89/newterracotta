...
Menu.prototype.init = function (params) {
	...
	this.nav = $(params.navigation, this.cont);
	this.items = $(params.items, this.nav);
	this.subClass = params.sub_menu;
	this.sub = $(this.subClass, this.items);

	this.items.bind('mouseenter', menuOver);
	this.items.bind('mouseleave', menuOut);
	this.resizable.bind('resize', function(){
		that.sub.height('auto');
	});

	function menuOver(e) {

		var sub = $(that.subClass, $(this));
		sub.stop(true, true);
		sub.slideDown();
