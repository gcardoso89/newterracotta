...
(function() {
	...
	Map.prototype._init = function( params ) {
		...
		this._listUrl = params.url || '/Portals/0/list.json';
		this._win = $( window );
		this._items = $( params.items || '.map-list address' );
		this._list = null;
		this._contTop = null;
			...
			if ( position ) {
				...
				if ( lat && lng ) {
					return {
						...
						click: function() {
							that._showElement( element.block );
						}
					}
				}
			}
	...
	Map.prototype._showElement = function( block ) {
		if ( block ) {
			this._items.hide();
			$( block ).show();
		}
